...
class Express extends BaseController
{
    ...
    public function freightTemplateList()
    {
        if (request()->isAjax()) {
            ...
            $shippingfee_list = new ExpressService();
            $expressCompanyinfo = $shippingfee_list->expressCompanyDetail($co_id);
            if(empty($expressCompanyinfo) || $expressCompanyinfo['shop_id'] != $this->instance_id)
            {
                $this->error("非法查询运费模板!");
            }
            if (! $co_id) {
    ...
    public function freightTemplateEdit()
    {
        ...
        if (request()->isAjax()) {
            ...
            $expressCompanyinfo = $express->expressCompanyDetail($co_id);
            if(empty($expressCompanyinfo) || $expressCompanyinfo['shop_id'] != $this->instance_id)
            {
                $this->error("非法操作运费模板!");
            }
