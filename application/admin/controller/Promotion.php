...
//             }
    ...
    public function mansongList()
    {
        if (request()->isAjax()) {
            ...
            //var_dump($operate_config);
            $list['examine_mansong_status'] = $operate_config->is_mansong_toExamine;
            return $list;
        }
        //获取满减送是否开启审核
        $config_service = new Config();
    ...
    public function updateMansong()
    {
        ...
        if(empty($info['mansong_id']) || $info['shop_id'] != $this->instance_id)
            {
                $this->error("满减送活动不存在或者当前用户无权限!");
            }
                $condition = array(
                    'shop_id' => $this->instance_id
                );
    ...
    public function getDiscountList()
    {
        if (request()->isAjax()) {
            ...
            $operate_config = $config_service->getOperateConfig();
            //var_dump($operate_config);
            $list['examine_discount_status'] = $operate_config->is_discount_toExamine;
            return $list;
        }
