...
class Goods extends BaseController
{
    ...
    public function getGoodslist(){
        $page_index = input('page');//页码
        $page_size = empty(trim(input('size'))) ? 3 : trim(input('size'));
        $condition = input();//条件获得新品
        unset($condition['page']);
        unset($condition['size']);
        if (!empty($condition['is_new'])&&$condition['is_new']==1){
            //新品的发布
            unset($condition['is_new']);
            $hotgoodsid = [];
            $platform = new Platform();
            $hot_list = $platform->getPlatformGoodsRecommend(1);//新品
            $hot_list = (collection($hot_list)->toArray());//转化为数组
            foreach ($hot_list as $hotgood){
                 array_push($hotgoodsid,$hotgood['goods_id'] );
            }
            $condition['goods_id'] = ['in',$hotgoodsid];
        }
        $goodsservice = new GoodsService();
        $goodslist = $goodsservice->getGoodsList($page_index,$page_size,$condition);//商品的列表
...
//        dump($hotgoodsid) ;
    ...
    public function getNewGoodslist(){
        $page_index = empty(trim(input('page'))) ? 1 : trim(input('page'));//页码
        $page_size = empty(trim(input('size'))) ? 3 : trim(input('size'));
        $condition = input();//条件获得新品
        unset($condition['page']);
        unset($condition['size']);
     ...
     * 商品详情
     *
     *  Ambigous <\think\response\View, \think\response\$this, \think\response\View>
     */
...
//        return view($this->style . 'Goods/goodsDetail');
    ...
    public function goodsClassificationList()
    {
        ...
        $category_tree_list = $goods_category->getGoodsCategoryTree(0);//分类导航树

//        var_dump(collection($goods_category->getGoodsCategoryTree(0))->toArray());
...
//        return view($this->style . 'Goods/goodsClassificationList');
        //dump(collection($goods_category_list_1['data'])->toArray());
    ...
    public function joinCartInfo()
    {
        ...
        $goods_detail = $goods->getGoodsDetail($goods_id);
        $this->assign("goods_detail", $goods_detail);
        //var_dump($goods_detail["spec_list"][0]);
    ...
    public function goodsSearchList()
    {
        /*if (request()->isAjax()) {*/
            ...
            $search_good_list = $goods->getGoodsList($page, 4, $condition, $order);
            $this->assign('search_good_list', $search_good_list);
    ...
    public function goodsList()
    {
        ...
        /*if (request()->isAjax()) {*/
            ...
            $category_id = isset($_REQUEST["category_id"]) ? $_REQUEST["category_id"] : ""; // 商品分类
            $brand_id = isset($_REQUEST["brand_id"]) ? $_REQUEST["brand_id"] : ""; // 品牌
            $order = isset($_REQUEST["order"]) ? $_REQUEST["order"] : ""; // 商品排序分类
            $sort = isset($_REQUEST["sort"]) ? $_REQUEST["sort"] : "desc"; // 商品排序分类
            $page = empty($_REQUEST["page"]) ? 1  : $_REQUEST["page"] ; //商品的页码
            $pid = empty($_REQUEST["pid"]) ? null :  $_REQUEST["pid"]; // 父亲id
            if (empty($pid)){
                $pid = $category_id;
                $this->assign('pid',$pid);
            }else{
...
//            var_dump($page);
            ...
            $goods = new GoodsService();
            $goods_list = $goods->getGoodsList($page, 4, $condition, $orderby);
            $this->assign('goodslist',$goods_list);


                $catlist = $goodscategory->getGoodsCategoryListByParentId($pid);//子类目录导航
