...
//        dump($goods_sku_list);
        ...
        if (! empty($address)) {
            ...
            // 物流公司
            $express_company_list = $goods_express_service->getExpressCompany(0, $goods_sku_list, $address['province'], $address['city']);
//             dump(Session::get('instance_id'));
        ...
        switch ($order_tag){
            // 立即购买 buygroup
            case "buy_now":
                $count_money = $order->getGoodsSkuListPrice($goods_sku_list);//商品单价
                break;
            case "buygroup":
                //一键拼单
                $count_money = $order->getGoodsSkuListGroupPrice($goods_sku_list);//拼团价格
                break;
            default :{
                $count_money = $order->getGoodsSkuListPrice($goods_sku_list);//商品单价
            }
        ...
        foreach ($list as $k => $v) {
            ...
            $list[$k]['group_price'] = sprintf("%.2f", $list[$k]['group_price']);//拼团价格
            //判断是否是拼团还是普通单价
            switch ($order_tag){
                // 立即购买 buygroup
                case "buy_now":
                ...
                case "buygroup":
                    //一键拼单
                    $list[$k]['subtotal'] = sprintf("%.2f", $list[$k]['group_price'] * $list[$k]['num']);//拼团价格
//                    dump(collection($list)->toArray());
        ...
        $member_account = $member->getMemberAccount($this->uid, 0);
    ...
    public function buyNowSession()
    {
        ...
        $cart_list["price"] = $member_price < $sku_info['promote_price'] ? $member_price : $sku_info['promote_price'];//普通价格
        $member_Groupprice = $goods_preference->getGoodsSkuMemberGroupPrice($sku_info['sku_id'], $this->uid);
        $cart_list["group_price"] = $member_Groupprice < $sku_info['group_price'] ? $member_Groupprice : $sku_info['group_price'];//拼团价格
