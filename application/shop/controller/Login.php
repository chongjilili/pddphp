    ...
    public function mobile()
    {
        ...
        if ($_POST) {
            ...
            if ($retval > 0 && $result) {
                ...
                $this->success("注册失败", __URL__ . "login/mobile");
            }
        }
        // 获取商场logo
        $website = new WebSite();
    ...
    public function email()
    {
        ...
        if ($_POST) {
            ...
            if ($retval > 0 && $result) {
                ...
                $this->success("注册失败", __URL__ . "/login/email");
            }
        }
        // 获取商场logo
        $website = new WebSite();
    ...
    public function registerBox()
    {
        ...
        if (trim($reg_config['register_info']) == "") {
            ...
            if (strpos($reg_config['register_info'], "plain") !== false) {
                $this->redirect(__URL__ . "/login/register");
            } elseif (strpos($reg_config['register_info'], "mobile") !== false) {
                $this->redirect(__URL__ . "/login/mobile");
            } elseif (strpos($reg_config['register_info'], "email") !== false) {
                $this->redirect(__URL__ . "/login/email");
            } else {
                $this->error("抱歉,商城暂未开放注册!");
            }
        }
