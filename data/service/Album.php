...
class Album extends BaseService implements IAlbum
{
    ...
    public function getAlbumClassList($page_index = 1, $page_size = 0, $condition = '', $order = '', $field = '*')
    {
        ...
        if(!empty($list['data']))
        {
            foreach ($list['data'] as $k => $v)
            {
                ...
                $pic_cover = "";
                if($v["album_cover"])
                $pic_info = $album_picture->getInfo(['album_id'=>$v['album_id'],"pic_id"=>$v["album_cover"]], 'pic_cover');
                if(!empty($pic_info)){
    ...
    public function deleteAlbumClass($aclass_id_array)
    {
        ...
        try {
            ...
            $condition = array(
                'shop_id' => $shop_id,
                'album_id' =>array('in', $aclass_id_array)
            );
            $album_info = $this->album_class->getInfo(["is_default"=>1],"album_id");
            $album_id = $album_info["album_id"];
            //删除所选相册
            $res = $this->album_class->destroy($condition);
            //将被删除相册下的图片转移到默认
            $this->album_picture->save(["album_id"=>$album_id],$condition);
    ...
    public function deletePicture($pic_id_array)
    {
        ...
        $res=1;
        if(!empty($pic_array)){     
            $user_img_array = $this->getGoodsAlbumUsePictureQuery(["shop_id"=>$shop_id]);
