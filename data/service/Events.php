...
class Events implements IEvents{
    ...
    public function mansongOperation(){
        ...
        try{
            ...
            $config = new Config();
            $operate_config = $config->getOperateConfig();
            if(!empty($operate_config->is_mansong_open)){  //平台开启了满减送才可进行
                
                if(!empty($operate_config->is_mansong_toExamine)){
                    $condition_start['examine_status'] = 1;
                }
    ...
    public function ordersClose(){
        ...
        try{
            ...
            $time = date("Y-m-d H:i:s", time()-$close_time*60);//订单自动关闭
            $condition = array(
                'order_status' => 0,
    ...
    public function ordersComplete(){
        ...
        try{
            ...
            $time = date("Y-m-d H:i:s", time()-3600*24*$complete_time);//订单自动完成
    ...
    public function discountOperation(){
        ...
        try{
            ...
            $condition_start = array('start_time' => array('ELT', $time),'status'   => 0);
            ...
            $config = new Config();
            $operate_config = $config->getOperateConfig();
            if(!empty($operate_config->is_discount_open)){  //平台开启了满减送才可进行
            
                if(!empty($operate_config->is_discount_toExamine)){
                    $condition_start['examine_status'] = 1;
                }
    ...
    public function autoDeilvery(){
        ...
        try{
            ...
            $time = date("Y-m-d H:i:s", time()-3600*24*$delivery_time);//订单自动完成
