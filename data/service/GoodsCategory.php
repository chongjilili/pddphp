...
class GoodsCategory extends BaseService implements IGoodsCategory 
{
     ...
     */
    public function getTreeCategoryList($pid=0,$show_deep='', $condition='')
    {
        ...
        foreach($one_list as $k1=>$v1){
            $two_list = array();
            $two_list = $this->getTreeCategoryList($v1['category_id']);
	...
	public function getGoodsCategoryTree($pid){
		...
		foreach($one_list as $k1=>$v1){
			$two_list = array();
			$two_list = $this->getGoodsCategoryTree($v1['category_id']);
			$one_list[$k1]['child_list'] = $two_list;
		}
		$list = $one_list;
		return $list;
