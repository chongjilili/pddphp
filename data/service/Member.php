...
class Member extends User implements IMember
{
    ...
    /**
     * (non-PHPdoc)
     * @see \data\api\IMember::getMemberShopsFavoritesList()
     */
...
//        var_dump($this->uid);
     ...
     */
    public function payMemberRecharge($out_trade_no)
    {
        ...
        if (! empty($pay_info)) {
            ...
            if ($pay_status == 1) {
                ...
                if (! empty($racharge_obj)) {
                    ...
                    $member_account = new MemberAccount();
                    $member_account->addMemberAccountData($pay_info["shop_id"], 2, $racharge_obj["uid"], 1, $racharge_obj["recharge_money"], 4, $racharge_obj["id"], "余额充值!");
                }
            }
        }
    }
    ...
    public function addMemberBalanceWithdraw($shop_id, $withdraw_no, $uid, $bank_account_id, $cash)
    {
        ...
        $data = array(
            ...
            'cash' => $cash,
            'ask_for_date' => date("Y-m-d H:i:s", time()),
            'status' => 0,
            'modify_date' => date("Y-m-d H:i:s", time())
        );
     ...
     * 添加意见反馈
     */
    public function addOpinion($uid, $opinion,$reply)
    {
    ...
    * 删除 
    */
    public function deleteCollect($fav_ids,$fav_type){
       $fav_ids_array = explode(',',$fav_ids);
       foreach ($fav_ids_array as $item){
           $this->deleteMemberFavorites($item, $fav_type);
       }
       return 1;
    }
}