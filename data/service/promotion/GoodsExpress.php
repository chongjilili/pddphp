...
class GoodsExpress extends BaseService{
     ...
     */
    public function getSkuListExpressFee($goods_sku_list, $express_company_id, $province, $city)
    {
        $fee = 0;
        ...
        if(!empty($goods_sku_list))
        {
            ...
            $shipping_template = $this->getShippingFeeTemplate($express_company_id, $province, $city);
     ...
     * @param unknown $city
     */
    private function getShippingFeeTemplate($express_company_id,$province, $city)
    {
        $shipping_fee = new NsOrderShippingFeeModel();
        $fee_array = $shipping_fee->getQuery(['co_id' => $express_company_id], '*', '');
        
       /* dump($express_company_id);
        echo $shipping_fee->getLastSql();*/
        $temp = array();
        $default = array();
    ...
    private function getSkuGroup($goods_sku_list)
    {
        ...
        foreach ($goods_sku_list_array as $k => $goods_sku_array)
        {
            ...
            $shipping_fee = $goods->getInfo(['goods_id' => $goods_id['goods_id']], 'shipping_fee,goods_weight,goods_volume,shipping_fee_type');
    ...
    */
    ...
    {
        $goods = new NsGoodsModel();
        $shipping_fee = $goods->getInfo(['goods_id' => $goods_id], 'shop_id, shipping_fee');
        if($shipping_fee['shipping_fee'] <= 0)
        {
            return "免邮";
        }else{
            $goods_sku_model = new NsGoodsSkuModel();
            $goods_sku = $goods_sku_model->getQuery(['goods_id' => $goods_id], 'sku_id', '');
            $express_company_list = $this->getExpressCompany($shipping_fee['shop_id'], $goods_sku[0]['sku_id'].':1', $province_id, $city_id);
     ...
     * @param unknown $city_id
     */
    public function getExpressCompany($shop_id, $goods_sku_list, $province_id, $city_id)
    {
 
        $express_company_model = new NsOrderExpressCompanyModel();
        $list = $express_company_model->getQuery(['shop_id' => $shop_id, 'is_enabled' => 1], 'co_id,company_name', '');
//        echo $express_company_model->getLastSql();
//        $list = collection($list)->toArray();
        if(!empty($list))
        {
            ...
            foreach ($list as $k => $v)
            {
                ...
                $express_fee = $this->getSkuListExpressFee($goods_sku_list, $v['co_id'], $province_id, $city_id);
//                dump($express_fee);
//                $express_fee = 0;
                if($express_fee >= 0)
                {
    ...
    {
        $express_company_model = new NsOrderExpressCompanyModel();
        $count = $express_company_model->getCount(['shop_id' => $shop_id]);
        return $count;
    }
}