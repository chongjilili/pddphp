...
class GoodsPreference extends BaseService{
    ...
    public function getGoodsSkuPrice($sku_id)
    {
        ...
        if(!empty($this->uid))
        {
            $member_price = $this->getGoodsSkuMemberPrice($sku_id, $this->uid);//会员价格
     ...
     * @param unknown $sku_id
     */
    public function getGoodsSkuListGroupPrice($sku_id)
    {
        $goods_sku = new NsGoodsSkuModel();
        $goods_sku_info = $goods_sku->getInfo(['sku_id' => $sku_id], 'goods_id,price,promote_price,group_price');
        if(!empty($this->uid))
        {
            $member_price = $this->getGoodsSkuMemberGroupPrice($sku_id, $this->uid);//拼团的会员价
            if($member_price < $goods_sku_info['group_price'])
            {
                return $member_price;
            }else{
                return $goods_sku_info['group_price'];
            }
        }else{
            return $goods_sku_info['group_price'];
        }
  ...
  /*   public function getPointMoney($point, $shop_id)
     ...
     * @param unknown $uid
     */
    public function getGoodsSkuMemberGroupPrice($goods_sku_id, $uid){
        //查询sku相关信息
        $goods_sku = new NsGoodsSkuModel();
        $sku_info = $goods_sku->getInfo(['sku_id'=> $goods_sku_id], 'group_price');
        $member_level_discount = $this->getMemberLevelDiscount($uid);
        return $sku_info['group_price']*$member_level_discount;
    }
