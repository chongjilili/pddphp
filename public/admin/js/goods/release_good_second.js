...
/**
 * 创建时间：2015年6月11日18:07:10 创建人：万域网络 功能说明：获取数据已对象方式存储
 */
function PackageProductInfo() {
	...
	productViewObj.group_price = $("#txtGroupPrice").val().replace(/^\s*/g, "").replace(/\s*$/g, "") == "" ? 0 : $("#txtGroupPrice").val().replace(/^\s*/g, "").replace(/\s*$/g,"");// 拼团价
	productViewObj.group_num = $("#txtGroupNum").val().replace(/^\s*/g, "").replace(/\s*$/g, "") == "" ? 0 : $("#txtGroupNum").val().replace(/^\s*/g, "").replace(/\s*$/g,"");// 拼团人数
	...
	productViewObj.shipping_fee_id = 0;// 运费模板编号
	var shopCategoryText = "";
	$("#productcategory-selected span").each(function() {
		shopCategoryText += $(this).attr("id") + ",";
	})
