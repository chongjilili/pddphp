...
$(function(){
	...
	var end_date = $("#endDate").val();
		$.ajax({
			type : "post",
			url : "PLATFORM_MAIN/member/accountdetail",
			async : true,
			data : {
...
// 				alert(JSON.stringify(data));
				...
				if (data["data"].length > 0) {
					for (var i = 0; i < data["data"].length; i++) {
						...
						html += '</tr>';
						html += '<input type="hidden" id="member_id" value="<?php echo $member_id; ?>"/>'
					}
				} else {
