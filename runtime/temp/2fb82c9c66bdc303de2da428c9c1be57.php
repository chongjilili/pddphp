...
function setGoodsSpecField(id, name, e){
	...
	$.ajax({
		type:"post",
		url:"PLATFORM_MAIN/goods/setgoodsspecfield",
		data:{
			'id' : id,
			'name' : name,
			'value' : value
...
function addGoodsSpec() {
	location.href = "PLATFORM_MAIN/goods/addgoodsspec";
}
$(function(){
	LoadingInfo(1);
});
	...
	function LoadingInfo(pageIndex) {
		...
		$.ajax({
			type : "post",
			url : "PLATFORM_MAIN/goods/goodsspeclist",
			async : true,
			data : {
				"pageIndex" : pageIndex
			},
			success : function(data) {
				...
				if (data["data"].length > 0) {
					for (var i = 0; i < data["data"].length; i++) {
						...
						html += '<td class="center"><a href="PLATFORM_MAIN/goods/updategoodsspec?spec_id=' + data["data"][i]["spec_id"] + '">修改</a>  <a href="javascript:delSpec(' + data["data"][i]["spec_id"] + ')">删除</a></td> ';
						html += '</tr>';
					}
				} else {
		...
		$( "#dialog" ).dialog({
			buttons: {
				"确定": function() {
					...
					$.ajax({
						type : "post",
						url : "PLATFORM_MAIN/goods/deletegoodsspec",
						data : {
						...
						success : function(data) {
							if (data['code'] > 0) {
	 							showMessage('success', data["message"], "PLATFORM_MAIN/goods/goodsspeclist");
							}else if(data['code'] == 0){
	 							showMessage('error', data["message"]);
							}else if(data['code'] == -1){
