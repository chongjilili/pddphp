<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:42:"template/platform\Config\notifyConfig.html";i:1500286961;s:27:"template/platform\base.html";i:1500286957;s:44:"template/platform\controlCommonVariable.html";i:1500286961;s:33:"template/platform\openDialog.html";i:1500286958;}*/ ?>
<!DOCTYPE html>
				...
				<?php if(is_array($notify_list) || $notify_list instanceof \think\Collection || $notify_list instanceof \think\Paginator): $i = 0; $__LIST__ = $notify_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$notify_obj): $mod = ($i % 2 );++$i;?>
					<tr style="border:1px solid #e5e5e5;border-top:0;background:none;">
						 ...
						 <?php if($notify_obj['is_use'] == 1): ?>
					  	<td style="text-align: center;background:none;" class="is_close_<?php echo $notify_obj['id']; ?>"><a href="javascript:;"  onclick="setIsUse(<?php echo $notify_obj['id']; ?>,0);"><img src="__ROOT__/public/admin/images/start.png" /></a></td>
					  	 <?php else: ?>
					  	<td style="text-align: center;background:none;" class="is_close_<?php echo $notify_obj['id']; ?>"><a href="javascript:;" onclick="setIsUse(<?php echo $notify_obj['id']; ?>,1);"><img src="__ROOT__/public/admin/images/close.png" /></a></td>
				  		 <?php endif; ?>					     	
					     <td style="text-align: center;background:none;">
					     	<?php if($notify_obj['key'] == 'EMAILMESSAGE'): ?>
					     	<span>
					  			<a href="PLATFORM_MAIN/config/messageemailconfig">配置</a>
					  		</span>
					  		<?php else: ?>
					  			<span>
					  				<a href="PLATFORM_MAIN/config/messagesmsconfig">配置</a>
					  			</span>
							<?php endif; if($notify_obj['key'] == 'EMAILMESSAGE'): ?>				  		  
					  		 <span><a href="PLATFORM_MAIN/config/notifyemailtemplate">模板</a></span>
					  		<?php else: ?>
					  		<span><a href="PLATFORM_MAIN/config/notifysmstemplate">模板</a></span>
...
function setIsUse(id,value){
	...
	$.ajax({
		type : "post",
        url: "PLATFORM_MAIN/config/updatenotifyenable",
        data: { "id": id, "is_use": is_use },
        async : false, // 让它同步执行
        success: function (data) {
        	if (data["code"] > 0) {
				showMessage('success', '提交成功');
				 window.location.href="PLATFORM_MAIN/config/notifyindex";
			}else{
