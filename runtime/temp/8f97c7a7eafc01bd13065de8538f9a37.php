...
function setAttributeField(id, name, e){
	...
	$.ajax({
		type:"post",
		url:"PLATFORM_MAIN/goods/setattributefield",
		data:{
			'id' : id,
			'name' : name,
			'value' : value
...
function addAttributeService() {
		location.href = "PLATFORM_MAIN/goods/addattributeservice";
	}
$(function(){
	LoadingInfo(1);
});
	...
	function LoadingInfo(pageIndex) {
		...
		$.ajax({
			type : "post",
			url : "PLATFORM_MAIN/goods/attributelist",
			async : true,
			data : {
				"pageIndex" : pageIndex
			},
			success : function(data) {
				...
				if (data["data"].length > 0) {
					for (var i = 0; i < data["data"].length; i++) {
						...
						html += '<td class="center"><a href="PLATFORM_MAIN/goods/updategoodsattribute?attr_id=' + data["data"][i]["attr_id"] + '">修改</a>  <a href="javascript:delAttr(' + data["data"][i]["attr_id"] + ')">删除</a></td> ';
						html += '</tr>';
					}
				} else {
		...
		$( "#dialog" ).dialog({
			buttons: {
				"确定": function() {
					...
					$.ajax({
						type : "post",
						url : "PLATFORM_MAIN/goods/deleteattr",
						data : {
							'attr_id' : attr_id
						},
						dataType : "json",
						success : function(data) {
							if (data['code'] > 0) {
	 							showMessage('success', data["message"],'PLATFORM_MAIN/goods/attributelist');
