...
function refresh_data(){
	...
	for (var i = 0; i < data.length; i++) {
		...
		html += '<td></td>';
		html +='<td class="text-center">'+data[i]["stock"]+'</td>';
		html += '<td class="discount_'+data[i]["goods_id"]+'" style="width:100px;"><input type="number" style="width:40px;text-align:center;margin-bottom:0;" name="discount" onchange="discount(this);" goodsid="'+data[i]["goods_id"]+'" value=""/><span style="margin-left:3px;margin-top-3px;">折</span></td>';
		html +='<td class="text-center goods-opt"><a href="javascript:;" class="btn btn-dange js-remove-reward" onclick="cancel_join('+data[i]["goods_id"]+',this)">取消参加</a></td></tr>';
	}
	$(".select-two tbody tr").remove();
	$(".select-two tbody").append(html);
